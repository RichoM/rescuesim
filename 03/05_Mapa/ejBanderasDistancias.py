...
wheelL.setVelocity(0)
wheelR.setVelocity(0)
meAlejeDelInicio=False
estoyCercaDelInicio=True
posInicial=gps.getValues()
robot.step(TIME_STEP)
irPorIzquierda=True
tiempoInicial=robot.getTime()

while robot.step(TIME_STEP) != -1:
    # Leo los sensores de distancia y guardo los valores en variables
    dist_left = ps5.getValue()
    dist_front = ps7.getValue()
    posActual=gps.getValues()

    # si mi distancia al inicio es mayor a determinado valor
    #   meAlejeDelInicio=True

    # si mi distancia al inicio es menor a determinado valor
    #  estoyCercaDelInicio=True
    # sino
    #  estoyCercaDelInicio=False

    if not meAlejeDelInicio:
        irPorIzquierda=True
    elif meAlejeDelInicio and estoyCercaDelInicio:
        irPorIzquierda=False 
    ...
    if irPorIzquierda:
        ...
        # Ante alguna condición yo tengo que pasar a irPorIzquierda=False
        # Por ejemplo, después de cierto tiempo
        # Otro ejemplo, más complicado, cuando vuelvo a estar en la posición incial
        
    else:
        #Programa todo para ir por Derecha
        pass